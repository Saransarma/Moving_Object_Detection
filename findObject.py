...
while True:
    ret, img = cam.read()  # Read from camera, check for success
    if not ret:  # Handle camera read errors
    ...
    if firstFrame is None:
        firstFrame = gaussianImg  # Capture first frame
        continue
    ...
    threshImg = cv2.dilate(threshImg, None, iterations=2)  # Dilate

    cnts = cv2.findContours(threshImg.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    cnts = imutils.grab_contours(cnts)

    for c in cnts:
        if cv2.contourArea(c) < area:  # Filter small contours
            continue
